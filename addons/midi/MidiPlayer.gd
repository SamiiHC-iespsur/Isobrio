...
func _ready( ):
	if AudioServer.get_bus_index( self.midi_master_bus_name ) == -1:
		...
		for i in range( 0, 16 ):
			...
			var cae: = GodotMIDIPlayerChannelAudioEffect.new( )
			var effect_indices_to_remove:Array[int] = []
			for k in range( AudioServer.get_bus_effect_count( midi_channel_bus_idx ) ):
				...
				if ae is AudioEffectPanner:
					cae.ae_panner = ae
				elif ( ae is AudioEffectReverb ) or ( ae is AudioEffectChorus ):
					effect_indices_to_remove.append( k )
			# Remove reverb/chorus effects if they already exist in the bus chain.
			for remove_i in range( len( effect_indices_to_remove ) - 1, -1, -1 ):
				AudioServer.remove_bus_effect( midi_channel_bus_idx, effect_indices_to_remove[remove_i] )
			cae.ae_reverb = null
			cae.ae_chorus = null
			if cae.ae_panner == null:
				cae.ae_panner = AudioEffectPanner.new( )
				AudioServer.add_bus_effect( midi_channel_bus_idx, cae.ae_panner )
			self.channel_audio_effects.append( cae )
...
func _apply_channel_reverb( channel:GodotMIDIPlayerChannelStatus ) -> void:
	# Reverb is disabled for dry playback.
	return

## チャンネルにコーラス適用
## @param	channel	チャンネルステータス
func _apply_channel_chorus( channel:GodotMIDIPlayerChannelStatus ) -> void:
	# Chorus is disabled for dry playback.
	return
